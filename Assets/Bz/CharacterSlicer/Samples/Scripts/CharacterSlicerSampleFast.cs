...
#pragma warning restore 0649
		...
		private void ConvertToRagdoll(GameObject resultNeg, GameObject resultPos, LazyActionRunner lazyRunnerNeg, LazyActionRunner lazyRunnerPos)
		{
			...
			ConvertToRagdoll(resultNeg, /*velocityContinue /*+ */new Vector3 (1, 1, 1), new Vector3(1, 1, 1)/*angularVelocityContinue*/, lazyRunnerNeg);
			ConvertToRagdoll(resultPos, /*velocityContinue/* - */new Vector3(-1, 1, 1), new Vector3(-1, 1, 1)/*angularVelocityContinue*/, lazyRunnerPos);
		}
		...
		static void SetVelocity(GameObject go, Vector3 velocityContinue, Vector3 angularVelocityContinue)
		{
			...
			for (int i = 0; i < rigids.Length; i++)
			{
                ...
                rigid.AddForce(velocityContinue.x * 500, velocityContinue.y*300, velocityContinue.z * 2500, ForceMode.Impulse);
            }
		}
		...
		public static IEnumerator SmoothDepenetration(GameObject go)
		{
			...
			for (int i = 0; i < rigids.Length; i++)
			{
                ...
                //    rigid.AddForce(100, 100, 300, ForceMode.Impulse);
            }
